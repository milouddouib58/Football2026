# app.py (النسخة الكاملة والنهائية مع مسار مطلق)
...
# هذا الجزء يحدد المسار المطلق للمجلد الذي يحتوي على app.py
# ثم يضيفه إلى قائمة المسارات التي يبحث فيها بايثون.
# هذا يضمن العثور على مجلد 'common' دائمًا.
...
# الآن ستعمل عمليات الاستيراد التالية بشكل صحيح
...
# (الكود الذي قدمته في المرة السابقة صحيح تمامًا من هذه النقطة فصاعدًا)
#
...
def load_teams_map() -> Optional[Dict]:
    # التأكد من أن المسار صحيح دائمًا
...
team1_name, team2_name, selected_home_id, selected_away_id = "", "", None, None
teams_map = load_teams_map()
...
if st.button("احسب التنبؤ الآن"):
    try:
        if not team1_name or not team2_name:
            st.warning("يرجى تحديد/كتابة اسمي الفريقين."); st.stop()
        ...
        with col1: st.metric("فوز المضيف", f"{p_home*100:.1f}%")
        with col2: st.metric("تعادل", f"{p_draw*100:.1f}%")
        with col3: st.metric("فوز الضيف", f"{p_away*100:.1f}%")
        with st.expander("مدخلات النموذج"): st.json(result["model_inputs"])
