...
from common.modeling import poisson_matrix_dc, matrix_to_outcomes, top_scorelines, suggest_goal_cutoff
...
class Predictor:
    ...
    def _select_season_key(self, comp_code: str) -> str:
        """يحاول استخدام الموسم الحالي، وإلا يعود للموسم السابق. ويتحقق من توفر جميع الموديلات."""
        year = current_season_year(datetime.now())
        candidates = [f"{comp_code}_{year}", f"{comp_code}_{year - 1}"]
        for key in candidates:
            if all(key in self.models[k] for k in ("elo", "factors", "averages", "rho")):
                return key
    ...
    def predict(self, team1_name: str, team2_name: str, comp_code: str, topk: int = 0, use_elo: bool = False) -> Dict:
        ...
        result = {
            ...
            "probabilities": {"home_win": float(p_home), "draw": float(p_draw), "away_win": float(p_away)}
        }

        if topk and topk > 0:
            tops = top_scorelines(matrix, top_k=topk)
            result["top_scorelines"] = [{"home_goals": i, "away_goals": j, "prob": float(p)} for i, j, p in tops]
